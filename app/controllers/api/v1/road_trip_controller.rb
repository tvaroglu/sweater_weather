class Api::V1::RoadTripController < ApplicationController
  def create
    if user_params[:api_key].blank? || ApiKey.find_by(value: user_params[:api_key]).blank?
      json_error_response(invalid_query, params_errors[:auth], :unauthorized)
    elsif user_params[:origin].blank? || user_params[:destination].blank?
      json_error_response(invalid_query, params_errors[:route])
    else
      ...
      forecast = ThirdPartyFacade.get_route_forecast(lat_lon.lat, lat_lon.lon, route.travel_time)
